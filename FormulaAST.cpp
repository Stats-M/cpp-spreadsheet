...
public:
    ...
    double Evaluate(const std::function<double(Position)>& func) const override
    {
        ...
        case Type::Divide:
            ...
            {
                // При делении на 0 выбрасывается ошибку вычисления FormulaError
                //throw FormulaError("Division by zero");
                throw FormulaError(FormulaError::Category::Div0);
            }
            break;
        default:
            //throw FormulaError("Unknown error in Evaluate()");
...
public:
    ...
    double Evaluate(const std::function<double(Position)>& func) const override
    {
        if (!cell_->IsValid())
        {
            // Координаты cell_ не валидны. Это ошибка типа #REF!
            throw FormulaError(FormulaError::Category::Ref);
        }

        // Возвращаем результат вычисления функтора для заданной ячейки
        // Единственный Evaluate(), который реально вычисляет при помощи функтора (лямбды),
        // используя для этого имеющийся указатель на индекс ячейки
        return func(*cell_);
    }
...
/*
* Два подхода для передачи параметров:
* 1) Можно передать ссылку на таблицу в функцию и делать с ней что угодно
* 2) Можно сделать маленький объект-функтор, который при конструировании хранит 
*    ссылку на таблицу, а в operator() умеет принимать индекс ячейки и возвращать 
*    соответствующее значение из таблицы, и тогда такой объект-функтор можно 
*    передать в метод, чтобы использовать его для получения доступа к ячейкам таблицы.
* У нас только CellExpr хранит адрес с индексом ячейки и только его Evaluate() 
* реально производит вычисления при помощи передаваемого функтора.
*/
